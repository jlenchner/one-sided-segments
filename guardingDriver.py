...
def deleteAllFilesInDir(directory):
    for f in os.listdir(directory):
        if(f.endswith(".eps")): #don't mess with subdirs
...
dataCenter.setGuardingModel(DataCenter.SOLVERS_CHOICE, DataCenter.ALL_BUT_DELTA_COVERAGE, delta=5)
#dataCenter.placeRandomAllVerticalRacks(NUM_RACKS)   #generate 10 random vertical racks
#dataCenter.placeRandomHStyleOrthogonalRacks(NUM_RACKS)
#dataCenter.placeHStyleRackConfiguration(NUM_RACKS)
dataCenter.placeRandomOrthogonalRacks(NUM_RACKS)
...
while iters < HARD_NUM_ITERATIONS:
    ...
    tt.clear()
    dataCenter.draw(tt, drawGrid=False, drawCandidateGuards=False)
    tt.setpos(-140, -380)
    tt.pendown()
